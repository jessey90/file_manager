...
class pager{
    ...
    function previous_page($previous_page_text,$page_url_last,$class_li){
        if($this->curent_page>1){
            $this->previous_page='<li><a href="'.$this->url.($this->curent_page-1).$page_url_last.'" class="'.$class_li.'">'.$previous_page_text.'</a></li>';
        }
    }
     function next_page($next_page_text,$page_url_last,$class_li){
        if($this->curent_page<$this->total_page){
            $this->next_page='<li><a href="'.$this->url.($this->curent_page+1).$page_url_last.'" class="'.$class_li.'">'.$next_page_text.'</a></li>';
        }
    }
    function firt_page($firt_page_text,$page_url_last,$class_li){
         if($this->curent_page>1){
            $this->firt_page='<li><a href="'.$this->url.'1'.$page_url_last.'" class="'.$class_li.'">'.$firt_page_text.'</a></li>';
        }
    }
    function last_page($last_page_text,$page_url_last,$class_li){
         if($this->curent_page<$this->total_page){
            $this->last_page='<li><a href="'.$this->url.$this->total_page.$page_url_last.'" class="'.$class_li.'">'.$last_page_text.'</a></li>';
        }
    }
